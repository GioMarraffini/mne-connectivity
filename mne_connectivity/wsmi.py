...
def wsmi(
    ...
    logger.info(
        f"""Computing wSMI and SMI for {n_unique_symbols} unique symbols
        (Numba-jitted)..."""
    )
    ...
    if n_channels_picked > 1:
        ...
        for epoch_idx in range(n_epochs):
            for conn_idx, (i, j) in enumerate(indices_list):
                wsmi_conn_data[epoch_idx, conn_idx, 0] = wsmi_val_epoched[
                    epoch_idx, i, j
                ]
    else:
        # For single channel or no channels, create empty connectivity
        logger.info(
            "Only 1 channel or fewer selected, wSMI connectivity will be empty."
        )
        wsmi_conn_data = np.empty((n_epochs, 0, 1))
        indices_list = []
