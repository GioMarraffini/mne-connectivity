...
@pytest.mark.skipif(not _test_data_available(), reason="wSMI test data not available")
def test_wsmi_against_test_data_all_cases():
    ...
    for test_case in test_data["tests"]:
        ...
        try:
            # Run our implementation using new explicit parameter API
            method_params = input_params["method_params"]
            conn = wsmi(
                epochs,
                kernel=kernel,
            ...
            assert expected_data.shape == new_data.shape, (
                f"Shape mismatch: Expected {expected_data.shape}, got {new_data.shape}"
            )
            ...
            print("  ✓ PASSED: Results match exactly")
            passed_tests += 1
    ...
    assert failed_tests == 0, (
        f"{failed_tests} test(s) failed out of {len(test_data['tests'])}"
    )
...
def test_wsmi_connectivity_patterns():
    ...
    for test_case in test_data["tests"]:
        ...
        print(
            f"""{scenario_name}: wSMI range [{np.min(conn_data):.4f},
            {np.max(conn_data):.4f}]"""
        )
