...
def test_wsmi_basic():
    ...
    conn = wsmi(epochs, kernel=3, tau=1, method_params={'filter_freq': 30.0, 'bypass_csd': True})
...
def test_wsmi_eeg_with_csd():
    ...
    # Test with CSD enabled (default behavior) - expect warnings about digitization points
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message="Only .* head digitization points")
        conn = wsmi(epochs, kernel=3, tau=1, method_params={'filter_freq': 25.0})
...
def test_wsmi_default_filter_frequency():
    ...
    info = create_info(ch_names, sfreq=sfreq, ch_types='eeg')
    epochs = EpochsArray(data, info, tmin=0.0)
    epochs.set_montage(mne.channels.make_standard_montage('standard_1020'), on_missing='ignore')
    
    # Test with no filter_freq specified (should use default: sfreq / (kernel * tau))
    kernel, tau = 3, 2
    expected_filter_freq = sfreq / (kernel * tau)  # 100 / (3 * 2) = 16.67 Hz
    
    conn = wsmi(epochs, kernel=kernel, tau=tau, 
                method_params={'bypass_csd': True})  # bypass CSD for simplicity
...
def test_wsmi_mixed_channel_types():
    ...
    # Test with CSD bypass (MEG channels present, CSD should only apply to EEG)
    conn = wsmi(epochs, kernel=3, tau=1, 
                method_params={'filter_freq': 30.0, 'bypass_csd': True})
...
def test_wsmi_bad_channels_interpolation():
    ...
    with warnings.catch_warnings():
        warnings.filterwarnings("ignore", message="Only .* head digitization points")
        warnings.filterwarnings("ignore", message="Setting channel interpolation method")
        conn = wsmi(epochs, kernel=3, tau=1, method_params={'filter_freq': 25.0})
...
def test_wsmi_input_validation():
    ...
    with pytest.raises(ValueError, match="filter_freq.*must be > 0 and < Nyquist"):
        wsmi(epochs, kernel=3, tau=1, method_params={'filter_freq': sfreq, 'bypass_csd': True})  # Above Nyquist
    
    # Test filter frequency of 0
    with pytest.raises(ValueError, match="filter_freq.*must be > 0 and < Nyquist"):
        wsmi(epochs, kernel=3, tau=1, method_params={'filter_freq': 0.0, 'bypass_csd': True})
...
def test_wsmi_memory_check():
    ...
    with pytest.raises(ValueError, match="could theoretically require.*GB"):
...
def test_wsmi_time_window():
    ...
    conn = wsmi(epochs, kernel=3, tau=1, tmin=0.5, tmax=2.0,
                method_params={'filter_freq': 30.0, 'bypass_csd': True})
...
def test_wsmi_insufficient_samples():
    ...
    with pytest.raises(ValueError, match="at least.*are needed"):
...
def test_wsmi_deterministic():
    ...
    conn1 = wsmi(epochs1, kernel=3, tau=1, method_params={'filter_freq': 25.0, 'bypass_csd': True})
    conn2 = wsmi(epochs2, kernel=3, tau=1, method_params={'filter_freq': 25.0, 'bypass_csd': True})
...
def test_wsmi_meg_data():
    ...
    # Should work with bypass_csd=True (MEG doesn't use CSD)
    conn = wsmi(epochs, kernel=3, tau=1, 
                method_params={'filter_freq': 30.0, 'bypass_csd': True})
...
def test_wsmi_no_data_channels():
    ...
    with pytest.raises(ValueError, match="No suitable channels.*found"):
        wsmi(epochs, kernel=3, tau=1, method_params={'bypass_csd': True})
...
def test_wsmi_csd_graceful_fallback():
    ...
    # Test with bypass_csd=True first (this should always work)
    conn_bypass = wsmi(epochs, kernel=3, tau=1, 
                      method_params={'filter_freq': 30.0, 'bypass_csd': True})
    
    assert conn_bypass.method == "wSMI"
    assert np.all(np.isfinite(conn_bypass.get_data()))
    ...
    with pytest.raises(RuntimeError, match="Cannot fit headshape"):
        wsmi(epochs, kernel=3, tau=1, 
             method_params={'filter_freq': 30.0})  # No bypass_csd
...
def _test_data_available():
    """Check if the new test data is available."""
    import os
    test_data_path = os.path.join(os.path.dirname(__file__), 'data', 'wsmi_test_data.pkl')
    return os.path.exists(test_data_path)


def _load_test_data():
    """Load the new wsmi test data."""
    import pickle
    import os
...
@pytest.mark.skipif(not _test_data_available(), 
                   reason="wSMI test data not available")
def test_wsmi_against_test_data_all_cases():
    ...
    across all 12 test cases, validating both algorithmic correctness and numerical precision.
    """
    ...
    print(f"\nTesting against {len(test_data['tests'])} test cases...")
    ...
    for test_case in test_data['tests']:
        ...
        print(f"\nTest {test_id}: {scenario_name} (kernel={kernel}, tau={tau})")
        
        try:
            # Run our implementation
            conn = wsmi(epochs, kernel=kernel, tau=tau, tmin=tmin, tmax=tmax, 
                       method_params=method_params, n_jobs=n_jobs, verbose=verbose)
            
            # Get our results
            new_data = conn.get_data()
            
            # Check shapes match
            assert expected_data.shape == new_data.shape, f"Shape mismatch: Expected {expected_data.shape}, got {new_data.shape}"
            
            # Check numerical agreement
            tolerance = 1e-10  # Very tight tolerance since it should be identical
            assert_allclose(expected_data, new_data, rtol=tolerance, atol=tolerance)
    ...
    print(f"\n=== SUMMARY ===")
    print(f"Passed: {passed_tests}/{len(test_data['tests'])}")
    print(f"Failed: {failed_tests}/{len(test_data['tests'])}")
    
    # All tests should pass
    assert failed_tests == 0, f"{failed_tests} test(s) failed out of {len(test_data['tests'])}"
    assert passed_tests == len(test_data['tests']), "Not all tests were executed"
...
def test_wsmi_linear_coupling_scenario():
    ...
    Linear coupling scenario models direct, proportional relationships between brain regions:
    ...
    - CH3: Weak linear coupling (CH3 = 0.3 × CH1 + noise) - moderate connectivity expected  
    ...
    linear_tests = [t for t in test_data['tests'] if t['scenario_name'] == 'linear_coupling']
    ...
    for test_case in linear_tests:
        ...
        # Run our implementation
        conn = wsmi(input_params['epochs'], kernel=input_params['kernel'], tau=input_params['tau'],
                   tmin=input_params['tmin'], tmax=input_params['tmax'], 
                   method_params=input_params['method_params'])
...
def test_wsmi_nonlinear_coupling_scenario():
    ...
    Nonlinear coupling scenario tests wSMI's key advantage over linear connectivity methods:
    ...
    nonlinear_tests = [t for t in test_data['tests'] if t['scenario_name'] == 'nonlinear_coupling']
...
def test_wsmi_network_coupling_scenario():
    ...
    Network coupling scenario models complex brain network topology with multiple connectivity patterns:
    ...
    - Network segregation (CH5 isolation)
    ...
    network_tests = [t for t in test_data['tests'] if t['scenario_name'] == 'network_coupling']
...
def test_wsmi_no_coupling_scenario():
    ...
    Clinical importance:
    - Pathological brain states (e.g., vegetative state) show reduced network connectivity
    - Normal brain also has functionally segregated regions with minimal interaction
    ...
    no_coupling_tests = [t for t in test_data['tests'] if t['scenario_name'] == 'no_coupling']
...
def test_wsmi_parameter_variations():
    ...
    - Fast dynamics (k3_t1) for acute states, slow dynamics (k3_t2) for chronic conditions
    ...
    # Group tests by parameter combinations
    param_combinations = {}
    for test_case in test_data['tests']:
        kernel = test_case['input_params']['kernel']
        tau = test_case['input_params']['tau']
        key = f"k{kernel}_t{tau}"
        
        if key not in param_combinations:
            param_combinations[key] = []
        param_combinations[key].append(test_case)
    
    print(f"\nTesting parameter combinations: {list(param_combinations.keys())}")
    
    for param_key, test_cases in param_combinations.items():
        print(f"\nTesting {param_key}: {len(test_cases)} scenarios")
...
def test_wsmi_performance_comparison():
    ...
    Expected: Execution time <10 seconds per test case, stable across parameter combinations.
    """
    import time
    ...
    for test_case in test_data['tests'][:3]:  # Test first 3 cases for performance
        ...
        performance_results.append((scenario_name, execution_time))
        
        print(f"{scenario_name}: {execution_time:.4f}s")
    
    # Just verify that it completes in reasonable time (not a strict requirement)
    avg_time = sum(time for _, time in performance_results) / len(performance_results)
    assert avg_time < 10.0, f"Average execution time too slow: {avg_time:.4f}s"
...
def test_wsmi_connectivity_patterns():
    ...
    This test validates the clinical and neuroscientific interpretability of wSMI results:
    ...
    for test_case in test_data['tests']:
        scenario_name = test_case['scenario_name']
        connectivity_info = test_case['connectivity_info']
        input_params = test_case['input_params']
        ...
        print(f"{scenario_name}: wSMI range [{np.min(conn_data):.4f}, {np.max(conn_data):.4f}]") 